...
def can_edit_user(viewer: User, target_user: User) -> bool:
    ...
    if viewer.is_rector:
        return True
    # Vice rector can edit employee profile in assigned department, but not role to superadmin or rector
    if viewer.is_vice_rector and target_user.department:
        is_scoped = viewer.department_responsibilities.filter(
            department=target_user.department,
            is_active=True,
        ).exists()
        if is_scoped and not target_user.is_rector and not target_user.is_superuser:
            return True
    # Dept head can edit details of department employee
    if viewer.is_department_head and viewer.department and target_user.department_id == viewer.department_id:
        if not target_user.is_rector and not target_user.is_vice_rector and not target_user.is_superuser:
            return True
...
class RectorRequiredMixin(AccessMixin):
    def dispatch(self, request, *args, **kwargs):
        ...
        if not request.user.is_rector:
            raise PermissionDenied(_('Only the Rector has permission to perform this action.'))
